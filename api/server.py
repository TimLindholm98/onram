...
def get_housekeeping():
    statement = 'SELECT * FROM test_suite WHERE datediff(now(), date_time) >= 5 AND power_state = "down"'
    cur = mysql.connection.cursor()
...
def delete_housekeeping():
    ...
    statement = 'DELETE FROM test_suite WHERE datediff(now(), date_time) >= 5 AND power_state = "down"'
    cur = mysql.connection.cursor()
    cur.execute( statement, )
    mysql.connection.commit()
    cur.close()
